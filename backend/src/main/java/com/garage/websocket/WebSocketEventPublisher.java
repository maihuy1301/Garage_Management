...
public class WebSocketEventPublisher {
    ...
    public void sendToBranch(String branchIdStr, RealtimeEvent event) {
        if (branchIdStr == null || event == null) {
            return;
        }
        try {
            log.info("Pushing WebSocket event to branch {}: {}", branchIdStr, event.getEventType());
            messagingTemplate.convertAndSend("/topic/branches/" + branchIdStr, event);
        } catch (Exception e) {
            log.warn("Failed to push WebSocket event to branch {}: {}", branchIdStr, e.getMessage());
        }
    }
}
