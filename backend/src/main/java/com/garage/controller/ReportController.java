...
public class ReportController {
    ...
    public ResponseEntity<ApiResponse<DashboardResponse>> getDashboard(
        ...
        DashboardResponse res = reportService.getDashboardReport(branchCode, from, to);
    ...
    public ResponseEntity<ApiResponse<RevenueReportResponse>> getRevenue(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to) {
        RevenueReportResponse res = reportService.getRevenueReport(branchCode, from, to);
    ...
    public ResponseEntity<ApiResponse<AppointmentReportResponse>> getAppointments(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to) {
        AppointmentReportResponse res = reportService.getAppointmentReport(branchCode, from, to);
    ...
    public ResponseEntity<ApiResponse<RepairOrderReportResponse>> getRepairOrders(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to) {
        RepairOrderReportResponse res = reportService.getRepairOrderReport(branchCode, from, to);
    ...
    public ResponseEntity<ApiResponse<List<ServiceReportResponse>>> getServices(
        ...
        List<ServiceReportResponse> res = reportService.getServiceReport(branchCode, from, to, limit);
    ...
    public ResponseEntity<ApiResponse<List<PartReportResponse>>> getParts(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to,
            @RequestParam(required = false, defaultValue = "10") Integer limit) {
        List<PartReportResponse> res = reportService.getPartReport(branchCode, from, to, limit);
    ...
    public ResponseEntity<ApiResponse<List<InventoryReportResponse>>> getInventory(
            @RequestParam(required = false) String branchCode) {
        List<InventoryReportResponse> res = reportService.getInventoryReport(branchCode);
    ...
    public ResponseEntity<ApiResponse<List<TechnicianReportResponse>>> getTechnicians(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to) {
        List<TechnicianReportResponse> res = reportService.getTechnicianReport(branchCode, from, to);
